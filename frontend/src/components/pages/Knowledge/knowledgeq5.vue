    ...
    <div class="stepper-panel">
      ...
      <div class="stepper-panels py-8">
        ...
        <div class="stepper-quest">
          ...
          <p class="font-semibold">5. Demonstrates - up - to - date knowledge and/or awareness on current trends and issues of the subject.</p>
          
          
        <div class="buttons">
        <button :id="[`index5`]" v-on:click="chose(5, 5, 10)" class="">5. Outstanding</button>
        <button :id="[`index4`]" v-on:click="chose(4, 5, 10)" class="">4. Very Satisfaction</button>
        <button :id="[`index3`]" v-on:click="chose(3, 5, 10)" class=""> 3. Satisfaction</button>
        <button :id="[`index2`]" v-on:click="chose(2, 5, 10)" class="">2. Fair</button>
        <button :id="[`index1`]" v-on:click="chose(1, 5, 10)" class="">1. Poor</button>
        </div>
          
        </div> 
        <div class=" ml-5 mt-10">
            <!--<div class="Preview">
            <a href="/Knowledgeq4"><button class=" btn button-next ">Preview</button></a> 
        </div>
            -->
            <div class="flex justify-between ml-4 mr-4">
        <div><a href="/knowledgeq4"><button class=" btn button-next " >Preview</button></a> </div>
...
const scoredata = {"eval_score": score,
    ...
    submitForm() {
      ...
      // alert(this.notice);
          ...
          window.location.href = window.location.origin + '/Independent?ratingPeriod='+ this.$route.query.ratingPeriod +'&evaluatee='+this.$route.query.evaluatee+'&evaluationType='+this.$route.query.evaluationType;
        }
    },
  }
}
