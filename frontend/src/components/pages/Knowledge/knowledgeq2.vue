    ...
    <div class="stepper-panel">
      ...
      <div class="stepper-panels py-8">
        ...
        <div class="stepper-quest">
          ...
          <p class="font-semibold">2. Draws and share information on the state on the art of theory and practice in his/her discipline.</p>
          
        <div class="buttons">
        <button :id="[`index5`]" v-on:click="chose(5, 2, 7)" class="">5. Outstanding</button>
        <button :id="[`index4`]" v-on:click="chose(4, 2, 7)" class="">4. Very Satisfaction</button>
        <button :id="[`index3`]" v-on:click="chose(3, 2, 7)" class=""> 3. Satisfaction</button>
        <button :id="[`index2`]" v-on:click="chose(2, 2, 7)" class="">2. Fair</button>
        <button :id="[`index1`]" v-on:click="chose(1, 2, 7)" class="">1. Poor</button>
        </div>
        ...
        <div><a href="/knowledge"><button class=" btn button-next " >Preview</button></a> </div>
...
const scoredata = {"eval_score": score,
    ...
    submitForm() {
      ...
      // alert(this.notice);
          ...
          window.location.href = window.location.origin + '/Knowledgeq3?ratingPeriod='+ this.$route.query.ratingPeriod +'&evaluatee='+this.$route.query.evaluatee+'&evaluationType='+this.$route.query.evaluationType;
        }
    },
  }
}
