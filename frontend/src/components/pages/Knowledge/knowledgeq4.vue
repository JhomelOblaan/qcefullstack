<template>
  ...
  <div>    <div class="stepper-content">
    ...
    <div class="stepper-panel">
      <div class="py-5">B. Knowledge of the subject matter</div>
    </div>
      <div class="stepper-panels py-8">
        ...
        <div class="stepper-quest">
          ...
          <p class="font-semibold">4.  Explains the relevance of present topics to the previous lessons, and relates the subject matter to relevant current issues and/or daily life activities.</p>
       
        <div class="buttons">
        <button :id="[`index5`]" v-on:click="chose(5, 4, 9)" class="">5. Outstanding</button>
        <button :id="[`index4`]" v-on:click="chose(4, 4, 9)" class="">4. Very Satisfaction</button>
        <button :id="[`index3`]" v-on:click="chose(3, 4, 9)" class=""> 3. Satisfaction</button>
        <button :id="[`index2`]" v-on:click="chose(2, 4, 9)" class="">2. Fair</button>
        <button :id="[`index1`]" v-on:click="chose(1, 4, 9)" class="">1. Poor</button>
        </div>
        ...
        <div class=" ml-5 mt-10">
            <!--<div class="Preview">
            <a href="/Knowledgeq3"><button class=" btn button-next ">Preview</button></a> 
        </div>
            -->
            <div class="flex justify-between ml-4 mr-4">
        <div><a href="/knowledgeq3"><button class=" btn button-next " >Preview</button></a> </div>
...
const scoredata = {"eval_score": score,
    ...
    submitForm() {
      ...
      // alert(this.notice);
          ...
          window.location.href = window.location.origin + '/Knowledgeq5?ratingPeriod='+ this.$route.query.ratingPeriod +'&evaluatee='+this.$route.query.evaluatee+'&evaluationType='+this.$route.query.evaluationType;
        }
    },
  }
}
