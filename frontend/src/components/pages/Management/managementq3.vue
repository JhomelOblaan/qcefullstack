      ...
      <div class="stepper-panels py-8">
        ...
        <div class="stepper-quest">
          <p class="font-semibold">3.  Designs and implements learning conditions and experience that promotes healthy exchange and/or confrontations.</p>
           ...
           <div class="buttons">
            <button :id="[`index5`]" v-on:click="chose(5, 3, 18)" class="">5. Outstanding</button>
            <button :id="[`index4`]" v-on:click="chose(4, 3, 18)" class="">4. Very Satisfaction</button>
            <button :id="[`index3`]" v-on:click="chose(3, 3, 18)" class=""> 3. Satisfaction</button>
            <button :id="[`index2`]" v-on:click="chose(2, 3, 18)" class="">2. Fair</button>
            <button :id="[`index1`]" v-on:click="chose(1, 3, 18)" class="">1. Poor</button>
...
const scoredata = {"eval_score": score,
    ...
    submitForm() {
      ...
      // alert(this.notice);
          ...
          window.location.href = window.location.origin + '/Managementq4?ratingPeriod='+ this.$route.query.ratingPeriod +'&evaluatee='+this.$route.query.evaluatee+'&evaluationType='+this.$route.query.evaluationType;
        }
    },
  }
}
