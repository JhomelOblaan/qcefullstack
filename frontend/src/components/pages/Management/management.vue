    ...
    <div class="stepper-panel">
      ...
      <div class="stepper-panels py-8">
        ...
        <div class="stepper-quest">
          <p class="font-semibold">1. Creates opportunities for intensive and/or extensive contribution of students in the class activities (e.g. breaks class into dyads, triads or buzz/task groups).</p>
          
          <div class="buttons">
            <button :id="[`index5`]" v-on:click="chose(5, 1, 16)" class="">5. Outstanding</button>
            <button :id="[`index4`]" v-on:click="chose(4, 1, 16)" class="">4. Very Satisfaction</button>
            <button :id="[`index3`]" v-on:click="chose(3, 1, 16)" class=""> 3. Satisfaction</button>
            <button :id="[`index2`]" v-on:click="chose(2, 1, 16)" class="">2. Fair</button>
            <button :id="[`index1`]" v-on:click="chose(1, 1, 16)" class="">1. Poor</button>
          </div>
        </div> 
        <div class=" ml-5 mt-10">
          <div class="flex justify-between ml-4 mr-4">
        <div><a href="/independentq5"><button class=" btn button-next " >Preview</button></a> </div>
...
const scoredata = {"eval_score": score,
    ...
    submitForm() {
      ...
      // alert(this.notice);
          ...
          window.location.href = window.location.origin + '/Managementq2?ratingPeriod='+ this.$route.query.ratingPeriod +'&evaluatee='+this.$route.query.evaluatee+'&evaluationType='+this.$route.query.evaluationType;
        }
    },
  }
}
