    ...
    <div class="stepper-panel">
      ...
      <div class="stepper-panels py-8">
        ...
        <div class="stepper-quest">
          <p class="font-semibold">2. Assumes roles as facilitator, resource person, coach, inquisitor, integrator, referee in drawing students to contribution to contribute to knowledge and understanding the concepts at hands.</p>
          
           <div class="buttons">
            <button :id="[`index5`]" v-on:click="chose(5, 2, 17)" class="">5. Outstanding</button>
            <button :id="[`index4`]" v-on:click="chose(4, 2, 17)" class="">4. Very Satisfaction</button>
            <button :id="[`index3`]" v-on:click="chose(3, 2, 17)" class=""> 3. Satisfaction</button>
            <button :id="[`index2`]" v-on:click="chose(2, 2, 17)" class="">2. Fair</button>
            <button :id="[`index1`]" v-on:click="chose(1, 2, 17)" class="">1. Poor</button>

          </div>
        </div> 
        <div class=" ml-5 mt-10">
            <!--<div class="Preview">
            <a href="/Management"><button class=" btn button-next ">Preview</button></a> 
        </div>
            -->
            <div class="flex justify-between ml-4 mr-4">
        <div><a href="/management"><button class=" btn button-next " >Preview</button></a> </div>
...
const scoredata = {"eval_score": score,
    ...
    submitForm() {
      ...
      // alert(this.notice);
          ...
          window.location.href = window.location.origin + '/Managementq3?ratingPeriod='+ this.$route.query.ratingPeriod +'&evaluatee='+this.$route.query.evaluatee+'&evaluationType='+this.$route.query.evaluationType;
        }
    },
  }
}
