  ...
  <div class="stepper-panel">
    ...
    <div class="stepper-panels py-8">
      ...
      <div class="stepper-quest ">
        ...
        <div class="buttons">
        <button :id="[`index5`]" v-on:click="chose(5, 1, 1)" class="">5. Outstanding</button>
        <button :id="[`index4`]" v-on:click="chose(4, 1, 1)" class="">4. Very Satisfaction</button>
        <button :id="[`index3`]" v-on:click="chose(3, 1, 1)" class=""> 3. Satisfaction</button>
        <button :id="[`index2`]" v-on:click="chose(2, 1, 1)" class="">2. Fair</button>
        <button :id="[`index1`]" v-on:click="chose(1, 1, 1)" class="">1. Poor</button>
        </div>
      
      </div> 
      
        <div class="flex justify-between ml-4 mr-4">
          <div><a href="/"><button class=" btn button-next " >Preview</button></a> </div>
...
const scoredata = {"eval_score": score,
    ...
    submitForm() {
      ...
      // alert(this.notice);
          ...
          window.location.href = window.location.origin + '/Commitmentq2?ratingPeriod='+ this.$route.query.ratingPeriod +'&evaluatee='+this.$route.query.evaluatee+'&evaluationType='+this.$route.query.evaluationType;
        }
    },
  }
}
