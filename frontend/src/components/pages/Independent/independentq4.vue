    ...
    <div class="stepper-panel">
      ...
      <div class="stepper-panels py-8">
        ...
        <div class="stepper-quest">
          <p class="font-semibold">4. Allows students to think independently and make their own decisions and holding them accountable for their performance based largely on their success in executing decisions.</p>
     
            <div class="buttons">
            <button :id="[`index5`]" v-on:click="chose(5, 4, 14)" class="">5. Outstanding</button>
            <button :id="[`index4`]" v-on:click="chose(4, 4, 14)" class="">4. Very Satisfaction</button>
            <button :id="[`index3`]" v-on:click="chose(3, 4, 14)" class=""> 3. Satisfaction</button>
            <button :id="[`index2`]" v-on:click="chose(2, 4, 14)" class="">2. Fair</button>
            <button :id="[`index1`]" v-on:click="chose(1, 4, 14)" class="">1. Poor</button>
            </div>
        
        </div> 
        <div class=" ml-5 mt-10">
            <!--<div class="Preview">
            <a href="/Independentq3"><button class=" btn button-next ">Preview</button></a> 
        </div>
            -->
          <div class="next">
...
const scoredata = {"eval_score": score,
    ...
    submitForm() {
      ...
      // alert(this.notice);
          ...
          window.location.href = window.location.origin + '/Independentq5?ratingPeriod='+ this.$route.query.ratingPeriod +'&evaluatee='+this.$route.query.evaluatee+'&evaluationType='+this.$route.query.evaluationType;
        }
    },
  }
}
