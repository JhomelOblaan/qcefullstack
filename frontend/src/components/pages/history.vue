                  ...
                  <table class="table-fixed">
                      ...
                      <tbody>
                        <tr v-for="(user, index) in filteredList" :key="index">
                          ...
                          <td> 
                            <button v-on:click="evaluate(user.rating_period.id, user.userevaluatee.id, user.evaluation_type.id)" class="">View Evaluation</button>
                          </td>
                        </tr>
                    ...
                    <div class="item error" v-if="!users.length">
                      <p class="pcenter">No results found!</p>
                    </div>
            </div>
      </div>
  </div> 
...
export default {
  ...
  methods:{
    getUsers(){
      ...
      axios.get(import.meta.env.VITE_VUE_APP_BASE_URL + '/evaluations/byStatus/1/' + user.id).then(res => {
        this.users = res.data;
        console.log(res);        
      });
    ...
    evaluate(ratingPeriod, evaluatee, evaluationType) {
          window.location.href = window.location.origin + '/view-evaluation?ratingPeriod='+ ratingPeriod +'&evaluatee='+ evaluatee+'&evaluationType='+evaluationType;
    },
  }
}
</script>
